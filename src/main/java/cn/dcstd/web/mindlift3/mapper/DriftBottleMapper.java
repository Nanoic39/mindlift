...
public interface DriftBottleMapper {
    ...
    @Select("SELECT * FROM drift_bottle_lib WHERE (id = #{id} AND userId = #{uid})")
    List<DriftBottleLibDO> getMyDriftBottleListWithId(@Param("id") int id, @Param("uid") int uid);
    ...
    @Insert("INSERT INTO drift_bottle_lib(content, userId, createTime) VALUES(#{content}, #{id}, NOW())")
    void sendDriftBottle(@Param("content") String content, @Param("id") int id);
}
