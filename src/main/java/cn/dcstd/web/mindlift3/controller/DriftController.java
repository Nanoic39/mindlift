...
public class DriftController {
     ...
     */
    @GetMapping("/received")
    public Result received(@RequestParam("bottle_id") int id) {
        List<DriftBottleReceivedVO> driftBottleReceivedVO = driftBottleService.getMyReceivedDriftBottle(id, TokenUtil.getCurrentUser().getId());
    ...
    public Result receivedAll() {
        List<DriftBottleReceivedVO> driftBottleReceivedVO = driftBottleService.getAllMyReceivedDriftBottle(TokenUtil.getCurrentUser().getId());
        return Result.success(driftBottleReceivedVO);
    }

    @PostMapping("/send")
    public Result sendDriftBottle(@RequestParam("content") String content) {
        driftBottleService.sendDriftBottle(content, TokenUtil.getCurrentUser().getId());
